...
/**
 ...
 7.0.0.2          -    -          7.0.0.3
  ...
  In each flow, 15 packets are sent.
  A NetAnim animation and a FLowMonitor file are recorded by the end of the simulation.
 */
NS_LOG_COMPONENT_DEFINE ("Use-case-1");
...
bool netDevCb(
  ...
  const Address &from)
{ 
    static Ptr<NormalRandomVariable> x = createNormalRandomVariable(50, 50, 25);;
...
int main (int argc, char *argv[])
{
  ...
  //uint16_t numberOfNodes = numberOfUENodes + numberOfeNBNodes;
  ...
  //  probeName = "ns3::ApplicationPacketProbe"; 
  ...
  Ptr<NetDevice> serverDevice = enbLteDevs.Get(0);
  Ptr<NetDevice> serverDevice2 = enbLteDevs.Get(1);
  Ptr<NetDevice> serverDevice3 = ueLteDevs.Get(0);
  Ptr<NetDevice> serverDevice4 = ueLteDevs.Get(1);
  Ptr<NetDevice> serverDevice5 = internetDevices.Get(0);
  ...
  serverDevice2->SetReceiveCallback(cb);
  serverDevice3->SetReceiveCallback(cb);
  serverDevice4->SetReceiveCallback(cb);
  serverDevice5->SetReceiveCallback(cb);
